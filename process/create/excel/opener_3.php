<?php
	...
	$missing = $n;
	$location = 5 - $missing;
	if($dt_start_time->format('H:i:s') >= $opener_emp_end[$i] && $dt_start_time->format('H:i:s') < $opener_emp_to[0])
	{
		if($location === $primary)
		{
	...
	else if($dt_start_time->format('H:i:s') >= $closer_emp_from[0] && $dt_start_time->format('H:i:s') < $opener_emp_end[0])
	{	
		$n = $secondary + (3 - $row);
		while($n > 4)
		{
