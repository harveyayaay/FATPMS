<?php
	...
	if($result_data = $result->fetch(PDO::FETCH_ASSOC))
	{
		...
		while($dt_start_time < $dt_end_time)
		{
			...
			echo '<td class="table-dark">'.$dt_start_time->format('H:i:s').' - '.$dt_to_time->format('H:i:s').'</td>';
			...
			while($primary < $primary_count+1)
			{
				...
				if($dt_search_time->format('H:i:s') >= '21:00:00')
				{
						...
						if($result_time_data = $result_time->fetch(PDO::FETCH_ASSOC))
						{
							...
							if($result_sched_data = $result_sched->fetch(PDO::FETCH_ASSOC))
							{
								...
								if($result_emp_data = $result_emp->fetch(PDO::FETCH_ASSOC))
								{
									$kind = 1;
									$pri = 1;
									$str = explode(':', $dt_search_time->format('H:i:s'));
									echo '<td>'.$p_primary.'<a href="" data-toggle="modal" data-target="#editModal" onclick="displayOptions2('.$Y.', '.$m.','.$d.', '.$str[0].', '.$str[1].','.$str[2].','.$pri.','.$primary.','.$kind.')" class="editsched">'.$result_emp_data['employee_lname'].', '.$result_emp_data['employee_fname'].'</a></td>';
								}	
					...
					if($task_found == false)
					{
						...
						$pri = 1;
						$str = explode(':', $dt_search_time->format('H:i:s'));
						echo '<td><a href="" data-toggle="modal" data-target="#addModal" onclick="displayOptions1('.$Y.', '.$m.','.$d.', '.$str[0].', '.$str[1].','.$str[2].','.$pri.','.$primary.','.$kind.')">Add Frontliner</a></td>';
					}		
				}
			...
			while($secondary < $secondary_count+1)
			{
				...
				if($dt_search_time->format('H:i:s') >= '21:00:00')
				{
						...
						if($result_time_data = $result_time->fetch(PDO::FETCH_ASSOC))
						{
							...
							if($result_sched_data = $result_sched->fetch(PDO::FETCH_ASSOC))
							{
								...
								if($result_emp_data = $result_emp->fetch(PDO::FETCH_ASSOC))
								{
									...
									$sec = 2;
									$str = explode(':', $dt_search_time->format('H:i:s'));
									echo '<td>'.$s_secondary.'<a href="" data-toggle="modal" data-target="#editModal" onclick="displayOptions2('.$Y.', '.$m.','.$d.', '.$str[0].', '.$str[1].','.$str[2].','.$sec.','.$secondary.','.$kind.')" class="editsched">'.$result_emp_data['employee_lname'].', '.$result_emp_data['employee_fname'].'</a></td>';
								}	
					...
					if($task_found == false)
					{
						...
						$sec = 2;
						$str = explode(':', $dt_search_time->format('H:i:s'));
						echo '<td><a href="" data-toggle="modal" data-target="#addModal" onclick="displayOptions1('.$Y.', '.$m.','.$d.', '.$str[0].', '.$str[1].','.$str[2].','.$sec.','.$secondary.','.$kind.')">Add Frontliner</a></td>';
					}		
				}
				else if($dt_search_time->format('H:i:s') <	'21:00:00')
				{	
