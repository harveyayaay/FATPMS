<?php
    ...
    $task_list = $Prod->getTaskList($id, $date, $date);
	...
	{	
		...
		echo '<tr>';
			...
			$task_avg = $Prod->getTaskAverageTime($id, $date, $date, $task_list_data['task_type']);
			$task_avg_data = $task_avg ->fetch( PDO::FETCH_ASSOC);
			echo '<td>' . $task_avg_data['average'] . '</td>';

			$task_vol = $Prod->countVolumeTaskType($id, $date, $date, $task_list_data['task_type']);
